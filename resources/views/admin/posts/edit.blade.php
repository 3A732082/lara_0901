    ...
    <div class="col-lg-12">
        <form action="/admin/posts/{{$post->id}}" method="POST" role="form">
            ...
            <div class="form-group">
                ...
                <input id="title" name="title" class="form-control" placeholder="請輸入文章標題" value="{{old('title',$post->title)}}">
            </div>
            ...
            <div class="form-group">
                ...
                <select id="is_feature"name="is_feature" class="form-control">
                    <option value="0"{{(!$post->is_feature?'seleted':'')}}>否</option>
                    <option value="1"{{($post->is_feature?'seleted':'')}}>是</option>
                </select>
            </div>
